  ...
  <div class="recommend-container">
    <div class="recommend-header">
      ...
      <div class="control-part">
        <span @click="slidePre" :class="{'disable':this.currPage === 0}" class="control control-left">
          <i class="fa fa-chevron-left icon-arrow"></i>
        </span>
        <span  @click="slideNext" :class="{'disable':this.currPage === Math.floor((this.recomends.length - 1) / 5)}" class="control control-right">
          <i class="fa fa-chevron-right icon-arrow"></i>
        </span>
      </div>
    </div>
    <div class="recommend-wrap">
      <ul class="recommend-content clearfix" :style="{ marginLeft: this.currentNum + 'px' }">
        <li class="recomend-item" v-for="item in recomends">
  ...
  methods: {
    slideNext () {
      if (this.currPage < Math.floor((this.recomends.length - 1) / 5)) {
        this.currentNum = this.currPage * (-1224)
        this.currPage++
      }
    },
    slidePre () {
