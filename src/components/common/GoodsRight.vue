    ...
    <ul class="goods-right-content">
      <template v-for="goods in currGoods">
        <li class="goods-right-item" v-if="$index < currGoods.length -1">
          <span :class="goods.discountType" class="discount-label">{{goods.discount}}</span>
            ...
            <p class="goods-price">
              {{goods.price}}元
              <span class="old-price">{{goods.oldPrice}}元</span>
            </p>
            <p class="goods-rank">
              {{goods.heat}}人评价
            </p>
            <div class="review" v-show="goods.reviewStatus">
